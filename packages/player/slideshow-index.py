...
class Slideshow(application.Application):
    ...
    def make_thumb_layout(self, dir_node):
    ...
    def prep_next_layout(self, dt=0):
        ...
        self.next_layout = self.make_thumb_layout(self.sources[self.position.next()])
        ...
        #self.next_layout = self.make_thumb_layout(self.sources[self.position].path)
    ...
    def prep_previous_layout(self, dt=0):
        ...
        self.previous_layout = self.make_thumb_layout(self.sources[self.position.previous()])
...
def main():
    ...
    node.scan_filetypes()
    #print node.images
    app.sources = node.sub_directories
    app.position = application.Position(len(app.sources))
    #app.position = 7
    #prep first layout:

    #app.prep_next_layout()
    app.next_layout = app.make_thumb_layout(app.sources[app.position.next()])
    #app.prep_previous_layout()
    app.previous_layout = app.make_thumb_layout(app.sources[app.position.previous()])
    app.current_layout = app.make_thumb_layout(app.sources[0])
